...
export class Banksy {
  ...
  async start(computeUnitsMax?: bigint, transactionAccountLockLimit?: bigint) : Promise<ProgramTestContext> {
  ...
  addProgram(name: string, programId: PublicKey) {
    this.inner.addProgram(name, programId.toBytes(), Keypair.generate().publicKey.toBytes());
  }

  addUpgradableProgram(name:string, programId:PublicKey, programAuthority: PublicKey, programData: PublicKey) {
    this.inner.addUpgradableProgram(name, programId.toBytes(), programAuthority.toBytes(), programData.toBytes());
  }

  addAccount(address: PublicKey, account: AccountInfoBytes) {
    this.inner.addAccount(address.toBytes(), fromAccountInfo(account));
  }

  addAccountWithLamports(address:PublicKey, owner: PublicKey, lamports:bigint) {
  ...
  addTokenMint(address:PublicKey, supply:bigint, decimals:number, mintAuthority?: PublicKey, freezeAuthority?: PublicKey) {
    this.inner.addTokenMint(
      address.toBytes(),
      supply,
      decimals,
      try_convert_from_pubkey(mintAuthority),
      try_convert_from_pubkey(freezeAuthority)
    );
  }

  addTokenAccount(address:PublicKey, mint:PublicKey, owner:PublicKey, amount:bigint, delegatedAmount:bigint, delegate?: PublicKey, isNative?: bigint, closeAuthority?: PublicKey) {
    this.inner.addTokenAccount(
      address.toBytes(),
      mint.toBytes(),
      owner.toBytes(),
      amount,
      delegatedAmount,
      try_convert_from_pubkey(delegate),
      isNative,
      try_convert_from_pubkey(closeAuthority)
    );
  }
}

function try_convert_from_pubkey(key?: PublicKey) : Uint8Array | undefined | null {
  if (key) {
    return key.toBytes();
  }
  return;
